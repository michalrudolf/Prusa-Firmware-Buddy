...
#define IS_LAN_STATIC(flg)        (flg & LAN_FLAG_ADDR_TYPE_POS)   // return true if flag set for static IP
#define IS_LAN_DHCP(flg)          (!IS_LAN_STATIC(flg))            // return true if flag set for DHCP
#define CHANGE_LAN_TO_STATIC(flg) (flg |= LAN_FLAG_ADDR_TYPE_POS)  // flip lan type flg to STATIC
#define CHANGE_LAN_TO_DHCP(flg)   (flg &= ~LAN_FLAG_ADDR_TYPE_POS) // flip lan type flg to DHCP
...
#define MATCH(s, n) strcmp(section, s) == 0 && strcmp(name, n) == 0
    ...
    if (MATCH("lan_ip4", "type")) {
        ...
        if (ip4addr_aton(value, &tmp_config->lan_ip4_addr)) {
            tmp_config->set_flag |= ETHVAR_MSK(ETHVAR_LAN_IP4_ADDR);
        }
    } else if (MATCH("lan_ip4", "mask")) {
        if (ip4addr_aton(value, &tmp_config->lan_ip4_msk)) {
            tmp_config->set_flag |= ETHVAR_MSK(ETHVAR_LAN_IP4_MSK);
        }
    } else if (MATCH("lan_ip4", "gateway")) {
        if (ip4addr_aton(value, &tmp_config->lan_ip4_gw)) {
            tmp_config->set_flag |= ETHVAR_MSK(ETHVAR_LAN_IP4_GW);
        }
...
void load_ini_params(ETH_Config_t *config_t) {
    ini_load_file(wui_ini_handler, (void *)config_t);
}

uint32_t save_eth_params(ETH_Config_t *config_t) {

    // type=STATIC/DHCP is in INI file
    if (config_t->set_flag & ETHVAR_MSK(ETHVAR_LAN_FLAGS)) {
    ...
    if (config_t->set_flag & ETHVAR_MSK(ETHVAR_HOSTNAME)) {
    ...
    if (config_t->set_flag & ETHVAR_MSK(ETHVAR_CONNECT_TOKEN)) {
        variant8_t token = variant8_pchar(config_t->connect_token, 0, 0);
        eeprom_set_var(EEVAR_CONNECT_TOKEN, token);
        //variant8_done() is not called, variant_pchar with init flag 0 doesnt hold its memory
    }
    if (config_t->set_flag & ETHVAR_MSK(ETHVAR_CONNECT_IP4)) {
